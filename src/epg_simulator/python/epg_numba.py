...
@njit
def get_t(fa, phi, pn):
    T = np.zeros((3, 3), dtype=numba.complex128)
...
def epg_single_alpha(        
    ...
    fa = np.ones((N_in,), dtype=numba.float64) * alpha
    fa[0] = abs(SP) * alpha

    N = len(fa)
    phi = np.ones(N) * -np.angle(SP)
    ...
    if rf_phase_mode == -1:
        ...
        pn = np.arange(1, N+1)#np.array(range(N)) + 1
    ...
    Omega_preRF = np.zeros((3, Np1), dtype=numba.complex128)
    Omega_postRF = np.zeros((3, N), dtype=numba.complex128)
    ...
    Xi_F_out = np.zeros((2 * N - 1, N), dtype=numba.complex128)  # F states, Eq.[27a]
    Xi_Z_out = np.zeros((N, N), dtype=numba.complex128)          # Z states, Eq.[27b]
    ...
    for pn in range(1, N + 1):  # Loop over RF pulse #pn
        ...
        if spoil:
            ...
            Omega_preRF[0, 1:] = Omega_preRF[0, 0:-1]       # dephase F+
            Omega_preRF[1, 0:-1] = Omega_preRF[1, 1:]       # dephase F-
...
def example(format: str = "numpy", plot: bool = True, verbose: bool = True):
    ...
    if format == "numpy":
        ...
        start = time.time()
        # Run simulation
        F0, Xi_F, Xi_Z = epg_single_alpha(Nfa, fa, tr, T1, T2, SP, spoil)
