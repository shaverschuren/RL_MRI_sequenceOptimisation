...
def epg(
    ...
    if type(alpha) == np.ndarray:
        phi = [-np.angle(SP)] * len(alpha)
        fa = [abs(SP) * alpha] * len(alpha)
    elif type(alpha) == float:
        phi = -np.angle(SP)
        fa = abs(SP) * alpha
    else:
        raise TypeError("alpha should be either a list or float!")

    for pn in range(2, N_in):  # The flip angle (array) is repeated N_in times
        fa = [fa, alpha]
    ...
    Omega_preRF = np.zeros(3, Np1)
    Omega_postRF = np.zeros(3, N)
    ...
    Xi_F_out = np.zeros(2 * N - 1, N)  # Xi_F with F+ and F- states, Eq.[27a]
    Xi_Z_out = np.zeros(N, N)          # Xi_Z with Z states, Eq.[27b]
    ...
    Xi_F_out[np.abs(Xi_F_out) < 1e-5] = 0
    Xi_Z_out[np.abs(Xi_Z_out) < 1e-5] = 0
