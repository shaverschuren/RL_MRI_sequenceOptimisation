...
class DQNCartPoleSolver():
    def __init__(
            self, n_episodes=1000, n_win_ticks=195,
            max_env_steps=None, gamma=1.0, epsilon=1.0,
            epsilon_min=0.01, epsilon_log_decay=0.995,
            alpha=0.01, alpha_decay=0.01, batch_size=64,
            monitor=False, quiet=False):
        self.memory = deque(maxlen=100000)
        self.env = gym.make('CartPole-v0')
        if monitor: self.env = gym.wrappers.Monitor(
    ...
    def run(self):
        ...
        for e in range(self.n_episodes):
            ...
            if e % 100 == 0 and not self.quiet:
                print(
                    f'[Episode {e}] - Mean survival time over last 100 '
                    f'episodes was {mean_score} ticks.'
                )
